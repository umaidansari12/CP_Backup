...
public:
	pair<int, bool> isSum(Node* root) {
		...
		if ((left.first + right.first) == root->data and left.second and right.second) {
			return {left.first + right.first + root->data, true};
		}

		return {left.first + right.first + root->data, false};
	}
